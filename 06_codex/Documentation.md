...
- 已修复暗色模式不彻底的问题：左侧编辑器工具栏、画布网格、尺标、文稿纸张、预览文字、表格和选区卡片都会切换到深海暗色。
- 已新增 `06_codex/DocumentEditingUXChecklist.md`：系统列出文稿编辑网站、UX 交互、OpenXML 修改链路和 DeepSeek 缓存策略的注意事项，并标注 Whale Editor 当前落地状态。
- 左侧工具栏新增“结构”视图：展示 DOCX/PPTX 解包、manifest、输入匹配、OpenXML patch、验证回写的完整管线。
- “结构”视图会展示当前文档类型、段落/表格/批注或幻灯片/形状/母版指标、关键 OpenXML 文件路径、缓存命中设计和 UX 护栏。

## 当前采用的统一口径

- 网页是 Office 风格工作台，不再是单页上传表单。
- 左侧 `document-page` 是网页内编辑/状态工作区；真实文件修改仍由 OOXML patch 写回并下载。
- 左侧工具栏现在是四种工作区视图：编辑、批注、审阅、结构；批注/审阅/结构读取真实 Office manifest，但不直接在浏览器里改原文件。
...
- DeepSeek 缓存策略固定为“稳定前缀 + 动态后缀”：system prompt、工具 schema、patch schema、manifest 稳定排序尽量放在可复用前缀；用户本轮指令、选区、批注处理结果和修复错误放在后缀。

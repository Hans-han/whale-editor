...
Upload a `.docx` / `.pptx` (or legacy `.doc` / `.ppt` via libreoffice), describe what you want changed (or attach a PDF / Markdown / Word format spec as reference), and Whale Editor's planner-executor agent will:

1. Scan the document into a structured manifest
2. Use a non-thinking model (`deepseek-chat`) to break your intent into 1–8 atomic patch steps
3. Run each step through the executor (your configured `deepseek-v4-flash` / `pro` / etc) with structured patch operations
4. Validate every patch against OOXML schema before committing
5. Stream progress live (NDJSON) and let you re-edit the same source file 100 times within a 10-min cache window — maximizing DeepSeek's KV-cache hit rate

The entire stable prompt prefix (system rules + tool spec + manifest + intent) is reused across reruns so you typically see **80–95 %+ cache hit rate** on follow-up edits.

> Built on a TypeScript/Node.js stack. Web UI ships in `apps/web/`; an experimental Office.js task-pane lives in `apps/office-addin/`.
